				...
				foreach ( $data as $event ):
					...
					<tr class="sp-row sp-post<?php echo ( $i % 2 == 0 ? ' alternate' : '' ); ?>">
						<td>
							...
							<span class="event-date"><?php echo get_the_time( get_option( 'date_format' ), $event ); ?></span>
							...
							<?php if ( $show_league ): $leagues = get_the_terms( $event, 'sp_league' ); if ( $leagues ): $league = array_shift( $leagues ); ?>
								<div class="event-league"><?php echo esc_html( $league->name ); ?></div>
							<?php endif; endif; ?>
							<?php if ( $show_season ): $seasons = get_the_terms( $event, 'sp_season' ); if ( $seasons ): $season = array_shift( $seasons ); ?>
								<div class="event-season"><?php echo esc_html( $season->name ); ?></div>
							<?php endif; endif; ?>
							<h4 class="event-title"><a href="<?php echo esc_url( get_post_permalink( $event, false, true ) ); ?>"><?php echo esc_html( $event->post_title ); ?></a></h4>
						</td>
					</tr>
					<?php
					$i++;
...
if ( $id && $show_all_events_link )
	echo '<a class="sp-calendar-link sp-view-all-link" href="' . esc_url( get_permalink( $id ) ) . '">' . __( 'View all events', 'sportspress' ) . '</a>';
?>