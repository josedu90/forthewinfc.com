...
<div class="sp-template sp-template-player-gallery">
	...
	foreach ( $groups as $group ) {
		...
		if($show_players) {
			echo '<ul class="player_gallery clearfix">';
		}
        foreach ($data as $player_id => $performance) {
            if (empty($group->term_id) || has_term($group->term_id, 'sp_position', $player_id)){
	...
	if ( $show_all_players_link )
		echo '<a class="sp-player-list-link sp-view-all-link" href="' . esc_url( get_permalink( $id ) ) . '">' . __( 'View all players', 'sportspress' ) . '</a>';
	?>
</div>